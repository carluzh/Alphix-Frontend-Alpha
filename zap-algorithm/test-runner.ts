/**
 * Test runner for REAL zap optimizer
 * Uses actual V4Quoter and V4Position SDK
 */
...
import { optimizeZapWithRealQuoter } from './real-zap-optimizer';
...
async function runTest(poolId: string, inputTokenSymbol: string, inputAmountStr: string) {
  console.log('\n' + '='.repeat(80));
  console.log(`Testing Real Zap Optimizer: ${poolId}`);
  ...
  console.log(`LP Fee: ${lpFeePercent.toFixed(4)}% (${lpFeeBps.toFixed(2)} bps)`);

  // Create real V4Pool SDK object
  ...
  // Run optimization
  console.log('\n' + '-'.repeat(80));
  console.log('Running Real Zap Optimization (with V4Quoter + V4Position SDK)...');
  console.log('-'.repeat(80) + '\n');

  const startTime = Date.now();
  const result = await optimizeZapWithRealQuoter(
    {
      v4Pool,
      inputToken,
      otherToken,
      ...
      lpFeePercent, // Pass actual fee for true price impact calculation
    },
    15 // max iterations
  );
  ...
  const truePriceImpact = Math.max(0, result.priceImpactPercent - lpFeePercent);
  ...
  console.log(`Fee: ${lpFeePercent.toFixed(4)}%`);
  console.log(`Observed Difference: ${result.priceImpactPercent.toFixed(4)}% (fee + slippage)`);
  console.log(`True Price Impact: ${truePriceImpact.toFixed(4)}% (slippage only)`);
  ...
  console.log(`Tested amounts: ${result.searchPath.map(a => `${(Number(a) / Number(inputAmount) * 100).toFixed(1)}%`).join(' → ')}`);
