...
import { BadgeCheck, InfoIcon, OctagonX } from "lucide-react"
  ...
  const handleConfirmSwap = useCallback(async () => {
    ...
    try {
      ...
      if (stateBeforeAction === "needs_approval") {
        ...
        toast("Confirm in Wallet", { icon: createElement(InfoIcon, { className: "h-4 w-4" }) })
      ...
      if (stateBeforeAction === "needs_signature") {
        ...
        toast("Sign in Wallet", { icon: createElement(InfoIcon, { className: "h-4 w-4" }) })
      ...
      toast("Confirm Swap", { icon: createElement(InfoIcon, { className: "h-4 w-4" }) })
