              ...
              <Card className="bg-card/60 border-border">
                <CardHeader className="pb-3">
                  <div className="flex items-center justify-between">
                    <CardTitle className="text-lg font-medium">{pools.find(p => p.id === selectedPool)?.pair || 'Pool Details'}</CardTitle>
                    <Badge variant="outline" className="bg-emerald-500/20 text-emerald-400 border-emerald-700/30">
                      {pools.find(p => p.id === selectedPool)?.apr}
                    </Badge>
                  </div>
                ...
                <CardContent>
                  <div className="grid grid-cols-2 gap-3 mb-4">
                    <div className="bg-muted/30 p-3 rounded-md">
                      ...
                      <div className="text-sm font-medium">{pools.find(p => p.id === selectedPool)?.liquidity}</div>
                    </div>
                    <div className="bg-muted/30 p-3 rounded-md">
                      <div className="text-muted-foreground text-xs mb-1">24h Volume</div>
                      <div className="text-sm font-medium">{pools.find(p => p.id === selectedPool)?.volume24h}</div>
                    </div>
                    <div className="bg-muted/30 p-3 rounded-md">
                      <div className="text-muted-foreground text-xs mb-1">Fees (24h)</div>
                      <div className="text-sm font-medium">{pools.find(p => p.id === selectedPool)?.fees24h}</div>
                    </div>
                    <div className="bg-muted/30 p-3 rounded-md">
