...
async function fetchProtocolVolumeHistory(): Promise<VolumeHistoryPoint[]> {
  const url = buildBackendUrlNoNetwork('/protocol/volume/history')
  ...
  try {
    ...
    const json: VolumeHistoryResponse = await response.json()
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const cacheKey = 'protocol:volume-history'

    const result = await cacheService.cachedApiCall(
      cacheKey,
      CACHE_TTL,
      () => fetchProtocolVolumeHistory(),
    ...
    console.error('[protocol/volume-history] Error:', error)
    return res.status(500).json({ success: false, error: 'Failed to fetch volume history' })
  }
}
