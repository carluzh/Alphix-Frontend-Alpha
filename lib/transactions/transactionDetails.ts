 ...
 *
 * Adapted from Uniswap's transaction types for Alphix.
 * Simplified to remove heavy dependencies while maintaining the same interface.
 *
...
export interface CurrencyAmount<T extends Currency> {
  currency: T
  quotient: bigint
  decimalScale: bigint
}
...
// Simplified for Alphix - we don't use GraphQL for transaction list
export type TransactionListQueryResponse = unknown
...
// Routing type - Alphix only uses CLASSIC
export enum Routing {
  CLASSIC = 'CLASSIC',
}

// Transaction extension for classic on-chain transactions
export interface ClassicTransactionExtension {
  routing: Routing.CLASSIC
  options: TransactionOptions
}

// Transaction types - Alphix uses only classic on-chain transactions
...
// For Uniswap compatibility - aliased to ClassicTransactionDetails
...
export interface BaseSwapTransactionInfo extends BaseTransactionInfo {
  ...
  protocol?: Protocol
  simulationFailureReasons?: string[]
  dappInfo?: DappInfoTransactionDetails
