 ...
 * to Uniswap's implementation for parity.
 *
 * @see interface/packages/uniswap/src/utils/polling.ts (Uniswap original)
 */

import { ONE_MINUTE_MS, ONE_SECOND_MS } from './time'

const TWO_SECONDS_MS = 2 * ONE_SECOND_MS
...
/**
 * Transaction details required for smart polling
 * Minimal interface matching Uniswap's TransactionDetails
 */
interface TransactionForPolling {
  receipt?: unknown
...
export function shouldCheckTransaction(
  ...
  transaction: TransactionForPolling,
): boolean {
