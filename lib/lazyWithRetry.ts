 ...
 * ```
 *
 * @see interface/apps/web/src/utils/lazyWithRetry.ts (Uniswap original)
 */
...
function attemptPageRefresh(): void {
  try {
    ...
    if (lastRefresh && now - parseInt(lastRefresh, 10) < REFRESH_COOLDOWN_DURATION) {
      console.warn('[lazyWithRetry] Page refresh skipped due to recent refresh (cooldown active)')
      return
    }

    localStorage.setItem(REFRESH_COOLDOWN_KEY, now.toString())
    console.log('[lazyWithRetry] Dynamic import failed after all retries, refreshing page...')
    window.location.reload()
  } catch (error) {
    // If localStorage fails, still try to refresh but log the issue
    console.warn('[lazyWithRetry] Failed to set refresh cooldown, refreshing anyway:', error)
    window.location.reload()
  }
}
    ...
    for (let attempt = 1; attempt <= options.maxRetries + 1; attempt++) {
      try {
        ...
        if (attempt > options.maxRetries) {
          // Log the error before handling the final failure
          console.error('[lazyWithRetry] Failed to load module after retries:', {
        ...
        console.warn(
          `[lazyWithRetry] Dynamic import failed (attempt ${attempt}/${options.maxRetries}), retrying in ${Math.round(delay)}ms:`,
          lastError.message,
        )
 ...
 * // Final failure: refresh page (unless disabled)
 * ```
 */
