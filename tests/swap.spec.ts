...
test.describe('Swap E2E Tests - Consolidated', () => {
  ...
  // SESSION 3: Multihop swaps (ExactIn and ExactOut) + Change button test
  test('Session 3: Multihop swaps (ExactIn, ExactOut, Change button)', async () => {
    console.log('\n' + '='.repeat(60))
    console.log('SESSION 3 STARTING: Multihop Swaps')
    console.log('  • Part 1: ExactIn Multihop (aUSDC → aUSDT → aETH)')
    console.log('  • Part 2: ExactOut Multihop (aUSDT → aUSDC → aBTC)')
    console.log('  • Part 3: ExactOut Change button test')
    console.log('='.repeat(60) + '\n')
    ...
    try {
      ...
      // Part 1: ExactIn Multihop (aUSDC → aUSDT → aETH)
      console.log('\n=== PART 1: ExactIn Multihop (aUSDC → aUSDT → aETH) ===')
      await executeSwapFlow({
        page,
        context,
      ...
      console.log('✓ ExactIn Multihop swap\n')

      // Part 2: ExactOut Multihop (aUSDT → aUSDC → aBTC)
      console.log('=== PART 2: ExactOut Multihop (aUSDT → aUSDC → aBTC) ===')
      let swapAgainBtn = page.getByRole('button', { name: 'Swap again' })
      await swapAgainBtn.click()
      await page.waitForTimeout(800)
      ...
      await executeSwapFlow({
        ...
        context,
        testAccount,
        fromToken: TOKENS.aUSDT,
        toToken: TOKENS.aBTC,
        amount: 0.00001,
        mode: 'exactOut',
      })
      console.log('✓ ExactOut Multihop swap\n')

      // Part 3: ExactOut "Change" button test (verify amounts don't change)
      console.log('=== PART 3: ExactOut Change Button Test ===')
      await swapAgainBtn.click()
      await page.waitForTimeout(800)
      ...
      // Select ExactOut mode
      const exactOutBtn = page.getByRole('button', { name: /exact out|output/i }).first()
      if (await exactOutBtn.isVisible().catch(() => false)) {
        await exactOutBtn.click()
        await page.waitForTimeout(500)
      }

      // Enter amount in the "to" field for ExactOut
      await toTokenInput.fill('0.00001')
      await page.waitForTimeout(2000) // Wait for quote

      // Capture the input amount before clicking Swap
      const inputAmountBefore = await swapAmountInput.inputValue()
      console.log(`Input amount before Swap: ${inputAmountBefore}`)
      ...
      // Capture the input amount after Change
      const inputAmountAfter = await swapAmountInput.inputValue()
      console.log(`Input amount after Change: ${inputAmountAfter}`)

      // Verify amounts haven't changed drastically
      const before = parseFloat(inputAmountBefore || '0')
      const after = parseFloat(inputAmountAfter || '0')
      const percentChange = Math.abs((after - before) / before) * 100

      if (percentChange > 5) {
        throw new Error(`Input amount changed by ${percentChange.toFixed(2)}% after clicking Change (${inputAmountBefore} → ${inputAmountAfter})`)
      }
      console.log(`Amount change: ${percentChange.toFixed(2)}% ✓ (within acceptable range)`)

      console.log('✓ Change button test passed\n')
